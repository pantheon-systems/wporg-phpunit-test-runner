 ...
 * errors is needed.
 *
 * When the XML data is missing, empty, unparseable, or carries no usable result
 * counts, execution stops with an error instead of returning an empty or hollow
 * result, because such a result is either rejected much later with a confusing
 * error or displayed as a pass. See
 * https://github.com/WordPress/phpunit-test-runner/issues/311.
 *
...
function process_junit_xml( $xml_string )
{
	...
	// Capture libxml's parser errors so an unparseable file reports *why* it is
	// unparseable, instead of fataling on $xml->xpath() further down. Restore the
	// previous handler state either way.
	...
	if ( false === $xml ) {
		$detail = '';
		if ( ! empty( $errors ) ) {
			$detail = ' First parser error: ' . trim( $errors[0]->message ) . ' at line ' . (int) $errors[0]->line . '.';
		}
	...
	// PHPUnit wraps everything in <testsuites>, but a root <testsuite> element is
	// also a legitimate JUnit shape from other producers.
	$project = 'testsuite' === $xml->getName() ? $xml : $xml->testsuite;
	$results = array();

	// All three counts are required: a payload carrying a test count but no
	// failure/error counts casts to zero downstream and renders as Passed.
	if (
